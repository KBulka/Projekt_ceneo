...
const getProducts = async (searchInput) => {
    ...
    if(await SearchInput.findOne({where: {searchInput: searchInput}}) === null)
    {
        SearchInput.create({searchInput: searchInput});    
        ...
        for (let i = 0; i < productCount; i++) {
            ...
            products.push({
                ...
                price: Number((cena.getElementsByClassName('value')[0].innerHTML + cena.getElementsByClassName('penny')[0].innerHTML).replace(/\s/g,'').replace(',','.')), 
